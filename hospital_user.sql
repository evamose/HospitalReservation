...
------------------------------------------------------------
-- 4-7. 예약 테이블 (RESERVATION) 추가
-- (기존 스크립트의 테이블 생성 부분 아래에 추가)
------------------------------------------------------------
CREATE TABLE RESERVATION (
    RES_ID       VARCHAR2(20)   PRIMARY KEY,       -- 예약번호 (예: 20251130-101)
    PATIENT_ID   NUMBER         NOT NULL,          -- 환자 ID (PATIENT 테이블 참조)
    DOCTOR_ID    NUMBER         NOT NULL,          -- 의사 ID (DOCTOR 테이블 참조)
    RES_DATE     DATE           DEFAULT SYSDATE,   -- 예약 날짜 및 시간
    MEMO         VARCHAR2(200),                    -- 증상 메모
    STATUS       VARCHAR2(20)   DEFAULT '예약완료', -- 상태 (예약완료/취소/진료완료)
    
    -- 외래키(FK) 설정: 환자와 의사가 실제로 존재해야만 예약 가능
    CONSTRAINT FK_RES_PATIENT FOREIGN KEY (PATIENT_ID) REFERENCES PATIENT(PATIENT_ID),
    CONSTRAINT FK_RES_DOCTOR FOREIGN KEY (DOCTOR_ID) REFERENCES DOCTOR(DOCTOR_ID)
);
...
------------------------------------------------------------
-- 5-7. 예약 테스트 데이터 (RESERVATION)
-- (기존 스크립트의 데이터 INSERT 부분 아래에 추가)
------------------------------------------------------------
-- 1번 환자(김가영)가 1번 의사(김민준)에게 내일 예약
INSERT INTO RESERVATION (RES_ID, PATIENT_ID, DOCTOR_ID, RES_DATE, MEMO, STATUS)
VALUES ('20251201-101', 1, 1, DATE '2025-12-01', '감기 기운이 있어 예약합니다.', '예약완료');

-- 2번 환자(박진수)가 2번 의사(이서준)에게 모레 예약
INSERT INTO RESERVATION (RES_ID, PATIENT_ID, DOCTOR_ID, RES_DATE, MEMO, STATUS)
VALUES ('20251202-205', 2, 2, TO_DATE('2025-12-02 14:30', 'YYYY-MM-DD HH24:MI'), '무릎 물리치료 예약', '예약완료');
------------------------------------------------------------
COMMIT;
------------------------------------------------------------
