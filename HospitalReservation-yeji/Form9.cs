...
namespace HospitalReservation
{
    public partial class Form9 : Form
    {
        ...
        {
            // [수정] txtPhone -> mtxtPhone으로 변경
            // mtxtPhone은 MaskedTextBox이므로 공백이 포함될 수 있어 Trim()이 필수입니다.
            if (string.IsNullOrWhiteSpace(txtPatientName.Text) || string.IsNullOrWhiteSpace(mtxtPhone.Text))
            {
                MessageBox.Show("이름과 전화번호를 모두 입력해주세요.");
                return false;
            }

            // [수정] 입력값 가져오기 (txtPhone -> mtxtPhone)
            ...
            try
            {
                using (OracleConnection conn = new OracleConnection(connectionString))
                {
                    ...
                    // 하이픈(-)이나 공백이 있어도 무시하고 숫자만 비교하는 강력한 쿼리
                    string query = @"
                SELECT PATIENT_ID 
                FROM PATIENT 
                WHERE TRIM(NAME) = :pname 
                  AND REPLACE(PHONE, '-', '') = REPLACE(:phone, '-', '')";
                    ...
                    // [확인용] 이제 진짜 번호가 뜰 겁니다! (확인 후 주석 처리하세요)
                    // MessageBox.Show($"검색 값 확인\n이름: {inputName}\n전화번호: {inputPhone}");
                    ...
                    {
                        // 정보가 없을 때
                        if (MessageBox.Show($"환자 정보를 찾을 수 없습니다.\n(입력값: {inputName}, {inputPhone})\n\n환자 등록 화면으로 이동하시겠습니까?",
                            "알림", MessageBoxButtons.YesNo) == DialogResult.Yes)
                        {
                            // 등록 폼 열기 (Form10이 등록 폼이라고 가정)
                            Form10 f = new Form10();
                            f.Show();
                        }
                        return false;
